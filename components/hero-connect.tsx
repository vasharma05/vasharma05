...
import type { ConnectLink } from "@/lib/content-types";
...
type IconDef =
  | { type: "fa"; icon: typeof faLinkedin }
  | { type: "svg"; Icon: React.ComponentType<{ className?: string }> };
...
export function HeroConnect({ label, links }: HeroConnectProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      <span className="text-xs font-medium tracking-wide text-[var(--muted)]">{label}</span>
      <div className="flex items-center gap-2">
        {visible.map((link) => {
          const key = link.id.toLowerCase().replace(/\s/g, "");
          const iconDef = ICON_MAP[key];
          return (
            <a
              key={link.id}
              href={link.url}
            ...
            >
              ...
              {!iconDef && <span className="text-xs font-semibold">{link.label.charAt(0)}</span>}
            </a>
          );
        })}
      </div>
